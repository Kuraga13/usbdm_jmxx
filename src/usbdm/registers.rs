...
fn read_core_reg_sequence(reg: DscRegisters) -> Result<Vec<u8>, BdmError> {
    if (reg as u8) < DSC_FIRST_CORE_REGISTER || (reg as u8) > DSC_LAST_CORE_REGISTER {
        return Err(BdmError::InternalError("Unexpected input value in read_core_reg_sequence".to_string()))
    }
    let reg_index: u8 = reg as u8 - DSC_FIRST_CORE_REGISTER;
...
fn write_core_reg_sequence(reg: DscRegisters, value: u32) -> Result<Vec<u8>, BdmError> {
    if (reg as u8) < DSC_FIRST_CORE_REGISTER || (reg as u8) > DSC_LAST_CORE_REGISTER {
        return Err(BdmError::InternalError("Unexpected input value in write_core_reg_sequence".to_string()))
    }
    let reg_index: u8 = reg as u8 - DSC_FIRST_CORE_REGISTER;
...
impl Programmer
{
    ...
    //
    pub fn dsc_read_core_reg(&self, reg: DscRegisters) -> Result<u32, BdmError> {
        if (reg as u8) < DSC_FIRST_CORE_REGISTER || (reg as u8) > DSC_LAST_CORE_REGISTER {
            return Err(BdmError::InternalError("Unexpected input value in read_core_reg".to_string())) 
        }
        ...
        if answer_vec.len() > 4 {return Err(BdmError::InternalError("Answer too long in read_core_reg".to_string()))}
        if answer_vec.len() == 0 {return Err(BdmError::InternalError("No answer in read_core_reg".to_string()))}
    ...
    /// `note` Leaves Core TAP in RUN-TEST/IDLE, EONCE register selected
    ///
    pub fn dsc_write_core_reg(&self, reg: DscRegisters, value: u32) -> Result<(), BdmError> {
        if (reg as u8) < DSC_FIRST_CORE_REGISTER || (reg as u8) > DSC_LAST_CORE_REGISTER {
            return Err(BdmError::InternalError("Unexpected input value in write_core_reg".to_string())) 
        }
    ...
    pub fn dsc_read_pc(&self) -> Result<u32, BdmError>{
        self.dsc_read_core_reg(DscRegisters::DscRegPc)
    }

    pub fn dsc_write_pc(&self, value: u32) -> Result<(), BdmError>  {
    ...
    //
    pub fn dsc_read_once_reg(&self, reg: DscRegisters) -> Result<u32, BdmError> {
        if (reg as u8) < DSC_FIRST_ONCE_REGISTER || (reg as u8) > DSC_LAST_ONCE_REGISTER {
            return Err(BdmError::InternalError("Unexpected input value in dsc_read_once_reg".to_string())) 
        }
        ...
        if answer_vec.len() > 4 {return Err(BdmError::InternalError("Answer too long in read_eonce_reg".to_string()))}
        if answer_vec.len() == 0 {return Err(BdmError::InternalError("No answer in read_eonce_reg".to_string()))}
    ...
    // @note Leaves Core TAP in RUN-TEST/IDLE
    //
    pub fn dsc_write_once_reg(&self, reg: DscRegisters, value: u32) -> Result<(), BdmError> {
        if (reg as u8) < DSC_FIRST_ONCE_REGISTER || (reg as u8) > DSC_LAST_ONCE_REGISTER {
            return Err(BdmError::InternalError("Unexpected input value in dsc_write_once_reg".to_string())) 
        }
