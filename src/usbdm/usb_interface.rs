...
pub async fn find_usbdm_as() -> Result<rusb::Device<rusb::GlobalContext>, BdmError>
{
    rusb::DeviceList::new()
    .unwrap()
    .iter()
    .filter(match_vid)
    .next()
    .ok_or(BdmError::Usb(rusb::Error::NotFound))
    
}

pub fn find_usbdm() -> Result<rusb::Device<rusb::GlobalContext>, BdmError>
{
    rusb::DeviceList::new()?
    .iter()
    .filter(match_vid)
...
impl  UsbInterface
{
    pub fn new(device: rusb::Device<rusb::GlobalContext>) -> Result<Self, BdmError> {
        let config = device.active_config_descriptor()?;
        let interface = config.interfaces().next();
        let interface_descriptor = interface.ok_or(BdmError::Usb(rusb::Error::NotFound))?.descriptors().next().ok_or(BdmError::Usb(rusb::Error::NotFound))?;
    ...
    fn check_usbm_return_code(&self, return_byte : &Vec<u8>) -> Result<(), BdmError> {
        ...
        if return_from_bdm != USBDM_ErrorCode::BDM_RC_OK {     
            return Err(BdmError::USBDM_Errors(return_from_bdm))
        } else {
            Ok (())
        }
    }

    pub fn control_transfer(&self, request_type: u8, request: u8, value: u16, index: u16, rx_size: usize) -> Result<Vec<u8>, BdmError> {
        let mut buff: Vec<u8> = vec![0; rx_size];
