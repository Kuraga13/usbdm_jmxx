...
/// Read id code.
//fn read_id_code(&self, commandRegLength :u8, resetTAP: bool) -> Result<(Vec<u8>), BdmError>;
...
impl From <u8>  for OnceStatus  {
    ...
    pub fn enableCoreTAP(prg:  &Programmer) -> Result<(), BdmError> {
        let mut sequence: Vec<u8> = Vec::new();
...
impl Programmer
{
    ...
    pub fn targetDebugRequest(&self) -> Result<OnceStatus, BdmError> {
        let mut sequence: Vec<u8> = Vec::new();
        sequence.push(JTAG_MOVE_IR_SCAN);                // Write enable EONCE command to IR
        sequence.push(JTAG_SET_EXIT_IDLE); 
        sequence.push(JTAG_SHIFT_IN_OUT_Q(JTAG_CORE_COMMAND_LENGTH));
    ...
    pub fn dsc_target_go(&self) -> Result<(), BdmError> {
