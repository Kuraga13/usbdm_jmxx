...
impl Programmer
{
    pub fn new(mut device : UsbInterface) -> Result<Self, BdmError> {
        let mut prog = Self {
            usb_device      : device,
            name            : "?".to_string(),
...
pub fn set_vdd(&mut self, power: TargetVddSelect ) -> Result<(), BdmError>{
...
pub fn force_vdd_off(&mut self ) -> Result<(), BdmError>{
...
pub fn get_power_state(&mut self) -> Result<PowerStatus, BdmError>
{
...
pub fn check_expected_power(&mut self, user_power_query : TargetVddSelect) -> Result<PowerStatus, BdmError>
{
...
pub fn set_vpp(&mut self, power: TargetVddSelect ) -> Result<(), BdmError>{
...
pub fn refresh_feedback(&mut self) -> Result<(), BdmError>
{
...
fn print_usbdm_programmer(&self) -> Result<(), BdmError>
...
pub fn set_target_mc56f(&mut self) -> Result<(), BdmError>{
...
pub fn set_settings(&mut self) -> Result<(), BdmError>{
...
///
fn set_speed(&mut self) -> Result<(), BdmError>{
   ...
   pub fn exec_jtag_seq(&self, mut jtag_seq : Vec<u8>,  answer_lenght : u8) -> Result<(Vec<u8>), BdmError>{
 ...
 pub fn jtag_reset(&mut self) -> Result<(), BdmError> {
 ...
 pub fn jtag_select_shift(&mut self, shift : u8) -> Result<(), BdmError> {
 ...
 pub fn jtag_write(&mut self, shift_exit : u8, cmd_lenght : u8, mut command : Vec<u8>) -> Result<(), BdmError> {
  ...
  pub fn bdm_control_pins(&mut self, control: u16) -> Result<(), BdmError>{
...
pub fn target_reset_low(&mut self) -> Result<(), BdmError>{
    const PIN_RESET_LOW   : u16 = 2<<2;   // Set Reset low
    self.bdm_control_pins(PIN_RESET_LOW)?;
    Ok(())
}

pub fn target_reset_release(&mut self) -> Result<(), BdmError>{
...
pub fn target_hardware_reset(&mut self) -> Result<(), BdmError>{
    const PIN_RESET_LOW   : u16 = 2<<2;   // Set Reset low
    const PIN_RELEASE     : u16 = 0xFFFF; // Release all pins (go to default for current target)
...
pub fn target_power_reset(&mut self, user_selected : TargetVddSelect) -> Result<(), BdmError>{
...
pub fn get_bdm_info(&mut self) -> Result<(), BdmError> {
    self.get_bdm_version()?;
    self.get_bdm_capabilities()?;
    Ok(())
}

fn get_bdm_version(&mut self) -> Result<(), BdmError>{
...
fn get_bdm_capabilities(&mut self) -> Result<(), BdmError>{
    let mut usb_buf = [0; 2];
    usb_buf[0] = 2;  // lenght
...
pub fn init_usbdm_for_mc56f(&mut self) -> Result<(), BdmError> {
