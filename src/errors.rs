...
impl fmt::Display for BdmError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
       write!(f, "{:?}", self)
    }
}

impl From<rusb::Error> for BdmError {
    fn from(error: rusb::Error) -> BdmError {
        BdmError::Usb(error)      
    }
}


impl From<packed_struct::PackingError> for BdmError {
    fn from(error: packed_struct::PackingError) -> BdmError {
        BdmError::PackerErr(error)      
    }
}
